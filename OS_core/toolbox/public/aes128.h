...
#ifndef OS_CORE_AES128_H
#define OS_CORE_AES128_H
...
const int Nr = 10;  //AES-128, 10轮加密
const int Nk = 4;   //密钥 word 个数, 此处为16字节
 ...
 *  加密
 */
void AES128_encrypt(byte in[4*4], word w[4*(Nr+1)])
{
    word key[4];
    for(int i=0; i<4; ++i)
        key[i] = w[i];
 ...
 *  解密
 */
void AES128_decrypt(byte in[4*4], word w[4*(Nr+1)])
{
    word key[4];
    for(int i=0; i<4; ++i)
        key[i] = w[4*Nr+i];
...
void Encrypt_AES128(void *buffer, void *data, unsigned int dat_len, char *key){
    byte tmp[16] = {0}, blank[16] = {0};
    byte k[16];
    word w[4*(Nr+1)];
    _core_toolbox_md5_get(key, (unsigned char*)k);
    KeyExpansion(k, w); //密钥扩展
    /** 分段加密 **/
    int i = 0;
    for(; i < dat_len / 16; i++){
        int t = 0;
        for(; t < 16; t++)
            tmp[t] = ((unsigned char *)data)[i*16+t];
        AES128_encrypt(tmp, w);
        for(t = 0; t < 16; t++)
            ((unsigned char *)buffer)[i*16+t] = (char)tmp[t].to_ulong();
    }
    i = dat_len / 16 * 16;
    int mod_c = dat_len % 16;
    if(mod_c){
        for(int t = 0; t < mod_c; t++){
            blank[t] = byte (((unsigned char *)data)[i+t]);
        }
        AES128_encrypt(blank, w);
        for(int t = 0; t < 16; t++) {
...
/** 注, 本函数参数dat_len的值应当是16的整数倍 **/
void Decrypt_AES128(void *buffer, void *data, unsigned int dat_len, char *key){
    byte tmp[16] = {0};// blank[16] = {0};
    byte k[16];
    word w[4*(Nr+1)];
    _core_toolbox_md5_get(key, (unsigned char*)k);
    ...
    /** 分段加密 **/
    int i = 0;
    for(; i < dat_len / 16 + (dat_len % 16 ? 1 : 0); i++){
        int t = 0;
        for(t; t < 16; t++)
            tmp[t] = ((unsigned char *)data)[i*16+t];
        AES128_decrypt(tmp, w);
        for(t = 0; t < 16; t++)
            ((unsigned char *)buffer)[i*16+t] = (char)tmp[t].to_ulong();
    }
    /*
